...
<div class="col-md-12">
    <div class="card">
        ...
        <form action="<?= site_url('admin/atlet/edit/') . $atlet['id']; ?>" method="post" enctype="multipart/form-data">
            <div class="card-body">
                <div class="row gy-3">
                    <?= csrf_field(); ?>

                    <div class="col-6">
                        <label class="form-label">Pilih Asal Sekolah</label>
                        <div class="icon-field">
                            <select name="sekolah" id="sekolah" class="select2 form-select" data-placeholder="Pilih Asal Sekolah" required>
                                <option></option>
                                <?php foreach ($sekolah as $s): ?>
                                    <option value="<?= $s['id']; ?>"
                                        <?= (old('sekolah') == $s['id'] || $atlet['sekolah_id'] == $s['id']) ? 'selected' : '' ?>>
                                        <?= $s['nama']; ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
